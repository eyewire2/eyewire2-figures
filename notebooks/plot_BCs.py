...
df = df.drop([
    "polar_dens_rt_res", "polar_dens_rz_res", "polar_dens_tz_res"], axis=1)  # Not needed
...
# %%
bc_preselection = [
    720575940547116305, 720575940550127573, 720575940550635615, 720575940550858976, 720575940553068893, 720575940554086469, 720575940554680920, 720575940563567755, # Glumi
    720575940546298641, 720575940546770705, 720575940547006481, 720575940547108369, 720575940547540497, 720575940578588460, # RBC
    720575940546300812, 720575940547112465, 720575940553161229, 720575940554789490, 720575940556965447, # XBC
    720575940547001105, 720575940549684436, # t1
    720575940546631185, 720575940547304977, 720575940548463494, 720575940578619346, # t2
    720575940547571473, 720575940549058321, 720575940549564177, 720575940550472790, # t3a
    720575940546609169, 720575940548367121, 720575940549292305, # t3b [720575940549796625]
    720575940548115656, 720575940549293585, 720575940549357780, 720575940551933170, # t4
    720575940546317829, 720575940546354193, 720575940546596881, 720575940549839586, # t5i
    720575940546829073, 720575940550809769, 720575940552050815, 720575940553059341, 720575940553768355, 720575940553903497, # t5o
    720575940547098385, 720575940549893435, 720575940551872169, # t5t [720575940550029585]
    720575940546549009, 720575940547555345, 720575940547667217, 720575940547755793, 720575940548136067, # t6
    720575940536773811, 720575940545571157, 720575940549740561, # t7
    720575940554530066, 720575940556610863, # t8
    720575940561798207, 720575940563161295, 720575940563426302, 720575940569796036, # t9

]

# %%
df.index.isin(bc_preselection).sum() / len(bc_preselection)
...
for ct in plot_order:
    rows = df[
        (df['celltype_final'] == ct)
        & df.valid_celltype_final
        # & df['status'].isin(['Complete (cut off)', 'Complete'])
    ]
    
    # Find convincing examples
    if sum(rows.index.isin(bc_preselection)) > 0:
        rows = rows[rows.index.isin(bc_preselection)].copy()
    if sum(rows.nuc.notnull()) > 0:
        rows = rows[rows.nuc.notnull()].copy()
    if sum(rows.cellclass_pred_prob > 0.95) > 0:
        rows = rows[(rows.cellclass_pred_prob > 0.95)].copy()
    if sum(rows.cellclass_pred_prob > 0.9) > 0:
        rows = rows[(rows.cellclass_pred_prob > 0.9)].copy()
    if sum(rows.celltype_pred_prob > 0.95) > 0:
        rows = rows[(rows.celltype_pred_prob > 0.95)].copy()
    if sum(rows.celltype_pred_prob > 0.9) > 0:
        rows = rows[(rows.celltype_pred_prob > 0.9)].copy()
    if sum(rows.post_has_soma > 0) > 0:
        rows = rows[(rows.post_has_soma > 0)].copy()
    if sum(rows.soma_rad_um < 10) > 0:
        rows = rows[(rows.soma_rad_um < 10)].copy()
    if sum(rows.soma_rad_um < 6) > 0:
        rows = rows[(rows.soma_rad_um < 6)].copy()
    if sum(rows.soma_rad_um > 2) > 0:
        rows = rows[(rows.soma_rad_um > 2)].copy()
    if sum(rows.soma_rad_um > 3) > 0:
        rows = rows[(rows.soma_rad_um > 3)].copy()
...
plot_embedding(
    ...
    dot_size=0.5,
    tsne_x_col="bc_tsne_d0",
    tsne_y_col="bc_tsne_d1",
    edgecolor="none",
    rasterized=True,
...
fig, axs = save_and_plot_feats(
    all_emb=df.loc[:, ['bc_tsne_d0', 'bc_tsne_d1']].values,
...
for celltype_final in plot_order:
    ...
    emb_ct = np.stack([rows['bc_tsne_d0'], rows['bc_tsne_d1']]).T
    ct_mean = np.mean(emb_ct, axis=0)
